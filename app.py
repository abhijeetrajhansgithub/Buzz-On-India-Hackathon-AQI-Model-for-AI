...
from algorithms.model.calculate_results_for_aqi import calculate_air_quality, calculate_air_quality_manually, calculate_air_quality_manually_final_suggested_verdict
...
def process_input():
    try:
        ...
        fetch_from__calculate_results_for_aqi = calculate_air_quality_manually(co, no, no2, so2, o3, nh3)
...
def send_message():
    ...
    if user_message.lower() in aqi_queries:
        ...
        html_message += f"<br><h3>Verdict: <b>{calculate_air_quality_manually_final_suggested_verdict(co, no, no2, o3, so2, nh3)}</b></h3>"
